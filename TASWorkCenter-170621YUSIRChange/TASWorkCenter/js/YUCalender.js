...
function setCld(SY, SM) {
	...
	for(i = 0; i < 42; i++) {
		...
		if(sD > -1 && sD < cld.length) {
			...
			lunar_obj.style.color = cld[sD].color;
			if(cld[sD].istoday) {
				solar_obj.style.background = '#fb0';
				solar_obj.style.color = '#ffffff';
				lunar_obj.style.background = '#fb0';
				lunar_obj.style.color = '#ffffff';
				// alert(i);
				addDay(i);
...
function addDay(v) {
	...
	if(solar_obj.innerHTML != '') {
		...
		fes = '<strong>' + cld[d].solarTerms + ' ' + cld[d].solar_festival + ' ' + cld[d].lunar_festival + '</strong>';

		//       day_detal= 
		//                '<table class="detallu"><tr><td>' +'<span>'+ cld[d].sy+'-'+cld[d].s_m+'-'+cld[d].s_d+' 星期'+cld[d].week+'<br>'+
		//                '<p style="font-family:courier;font-size: 54px;margin-right:5%;margin-bottom: 2%;">'+cld[d].s_m+'-'+cld[d].s_d+'</p>'+
		//                '<span style=" float: right;margin-top: -6%;font-size: 13px;">'+ fes +'农历'+(cld[d].isLeap?'闰 ':' ')+cld[d].l_m+' 月 '+cld[d].l_d+' 日</span><br>'+
		//                '<span style="float: right;margin-top: -1%;font-size: 13px;">'+cld[d].c_y+'年 '+cld[d].c_m+'月 '+cld[d].cal_d + '日</span>'
		//                '</td></tr></table>';
		day_detal = '<table class="detallu"><tr><td><span>' + cld[d].sy + '-' + cld[d].s_m + '-' + cld[d].s_d + ' 星期' + cld[d].week + '</span></td></tr>' +
			'<tr><td><span><strong style:"font-size:30px">'+animal_year+'</strong>年    农历' + (cld[d].isLeap ? '闰 ' : ' ') + cld[d].l_m + ' 月 ' + cld[d].l_d + ' 日</span></td></tr>' +
			'<tr><td><span>' + cld[d].c_y + '年 ' + cld[d].c_m + '月 ' + cld[d].cal_d + '日</span></td>' +
			'<tr><td style:"width:100%;">' + fes + '</td></table>'; //节日提示
