...
(function ($) {
    ...
    $.fn.comboboxfilter.methods = {
        ...
        init: function (target, ops) {
            ...
            listcontainer.append('<ul class="list"></ul> <span class="J_FilterMore filter-more"><span class="open">更多</span><span class="closeup">收起</span><i></i></span>');
            listcontainer.find('.open').unbind('click').bind('click', function() {//绑定点击更多事件
                $(target).removeClass('hotel-filter-list-min');
            });
            listcontainer.find('.closeup').unbind('click').bind('click', function () {//绑定点击更多事件
