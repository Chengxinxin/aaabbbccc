...
jQuery.fn.rater = function(options) {
	...
	content.find('.rater-star-item').mouseover(function() {
		...
		//
		var Array = jQuery(this).prevAll('.rater-star-item');
		var current_number = Array.length + 1;
...
$(function() {
	var options = {
		...
		info_format: function(value) {
			...
			switch(value) {
				case 1:
					info = '<div class="info-box">1分&nbsp;很不满意<div>工作质量和服务态度都非常差，太令人失望了！</div></div>';
					break;
				case 2:
					info = '<div class="info-box">2分&nbsp;不满意<div>工作质量效果不好，不能满足客户要求。</div></div>';
					break;
				case 3:
					info = '<div class="info-box">3分&nbsp;一般<div>工作完成质量一般，服务完成质量一般。</div></div>';
					break;
				case 4:
					info = '<div class="info-box">4分&nbsp;满意<div>工作和服务完成情况都比较满意，符合我的期望。</div></div>';
					break;
				case 5:
					info = '<div class="info-box">5分&nbsp;非常满意<div>我很高兴！工作和服务治疗都超出我的预期，太棒了！</div></div>';
					break;
				default:
					info = value;
					break;
...
function staticStarGarde(starnumber, starstring) {
	//设置评论固定星星的。一个星星宽24px
	var starNumber = starnumber;
	var starString = starstring;
	//赋值星星长度
	$(".rater-star-item").prevAll('.rater-star-item-current').css('width', 24 * starNumber);
	//去除提示等
	$(".rater-star-item").parents(".rater-star").find(".rater-star-item-tips").remove();
	$(".rater-star-item").parents(".goods-comm-stars").find(".rater-click-tips").remove();
	//四颗星和三颗星
	if(parseInt($(".rater-star-item").prevAll('.rater-star-item-current').css("width")) == 96 || parseInt($(".rater-star-item").prevAll('.rater-star-item-current').css("width")) == 72) {
		$(".rater-star-item").prevAll('.rater-star-item-current').addClass('rater-star-happy');
	} else {
		$(".rater-star-item").prevAll('.rater-star-item-current').removeClass('rater-star-happy');
	}
	//五颗星
	if(parseInt($(".rater-star-item").prevAll('.rater-star-item-current').css("width")) == 120) {
		$(".rater-star-item").prevAll('.rater-star-item-current').addClass('rater-star-full');
	} else {
		$(".rater-star-item").prevAll('.rater-star-item-current').removeClass('rater-star-full');
	}
	//固定显示星数需要解绑各种事件
	$(".rater-star").unbind(); //移除所有 
	$(".rater-star-item").unbind(); //移除所有 
	$('.rater-star-result').text(starString);
}